---
aliases: 
- Skelette
tags:
- Kreatur/Untot/Skelett
---
```statblock
image: [[Wikilink To Image]]
name: Skelett
size: [[Mittelgroß]]
type: [[Untote|Untoter]]
subtype: ([[Skelette|Skelett]])
alignment: [[Rechtschaffen Böse]]
ac: 13 + 2 ([[Lederrüstung|Rüstungsfetzen]], [[Holzschild]])
sr: 0 + 1 ([[Holzschild]])
hp: 13
speed: 9 m (6 Kästchen)
stats: [10, 14, 15, 6, 8, 5]
damage_vulnerabilities: [[Wuchtschaden|Wucht]]
damage_immunities: [[Giftschaden|Gift]]
condition_immunities: [[Erschöpfung]], [[Vergiftet]]
senses: [[Dunkelsicht]], [[Wahrnehmung#Passive Wahrnehmung]] 9
languages: Versteht alle Sprachen, die es im Leben kannte, kann aber nicht sprechen
cr: 1/4
actions:
  - name: [[Kurzschwert]]
    desc: Nahkampf-Waffenangriff +4 zum Treffen, Reichweite 1,5 m (1 Kästchen), ein Ziel. Treffer 5 (2W3+2)
reactions:
  - name: Parieren
    desc: Das Kurzschwert verleiht dem Skelett die Eigenschaft [[Parieren]] als Reaktion.
```